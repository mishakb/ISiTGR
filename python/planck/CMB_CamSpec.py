# simplified pure python version of CamSpec. Not yet inherited from CMBlikes
# but does need cosmomc:python in path to import CMBlikes.ClsArray type
# see main function at bottom for example of usage
from __future__ import absolute_import
from __future__ import print_function
import numpy as np
from CMBlikes import ClsArray, fast_chi_squared
...
class CamSpec_likelihood(object):
    ...
    def __init__(self, dataset, dataset_params={}, silent=False):
        ...
        self.cl_used = np.array([name in use_cl for name in self.cl_names], dtype=bool)
        ...
        for i, (cal, n) in enumerate(zip(cals, self.used_sizes)):
            if n > 0:
                if i <= 3:
                    ...
                    delta_vector[ix:ix + n] -= ClArray.get((0, 1))[self.ell_ranges[i]] / cal
                elif i == 5:
                    delta_vector[ix:ix + n] -= ClArray.get((1, 1))[self.ell_ranges[i]] / cal
                ix += n
        if fast_chi_squared is not None:
            return fast_chi_squared(self.covinv, delta_vector)
        else:
            return self.covinv.dot(delta_vector).dot(delta_vector)
    ...
    def get_weights(self, data_params):
        ...
        weights = []
        for i in range(4):
            ells = camspec.ell_ranges[i]
            vec = np.zeros(camspec.data_vector.shape)
...
if __name__ == "__main__":
    ...
    likeTT = CamSpec_likelihood(dataset, {'use_cl': '100x100 143x143 217x217 143x217'})

    for base in ['base', 'base_Alens']:
        for camspec, data in zip([likeTT, like], ['CamSpecHM_TT_lowl_lowE', 'CamSpecHM_TTTEEE_lowl_lowE']):
            root = base + '_' + data
            fit = BestFit(os.path.join(dropbox, 'Bestfits/%s.minimum' % root), want_fixed=True)
            cls = ClsArray(os.path.join(dropbox, 'Bestfits/%s.minimum.theory_cl' % root))
            start = time.time()
            ...
            for v in fit.chiSquareds:
                if 'CamSpec' in v[1].name:
                    test_chi2 = v[1].chisq
                    print('Chi-squared calculated: %s, best-fit %s' % (chi2, test_chi2))
                    break
