from .baseconfig import isitgrlib, CAMBError, CAMBValueError, CAMBUnknownArgumentError, np
...
def run_ini(ini_filename, no_validate=False):
    ...
    run_inifile = isitgrlib.__camb_MOD_camb_runinifile
...
def read_ini(ini_filename, no_validate=False):
    ...
    if not no_validate:
        validate_ini_file(ini_filename)
    cp = model.CAMBparams()
    read_inifile = isitgrlib.__camb_MOD_camb_readparamfile
...
CAMB_GetAge = isitgrlib.__camb_MOD_camb_getage
