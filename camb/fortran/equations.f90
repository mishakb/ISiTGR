    ...
    subroutine MassiveNuVarsOut(EV,y,yprime,a,adotoa,grho,gpres,dgrho,dgq,dgpi, dgpi_diff,pidot_sum,clxnu_all,dgpi_3wplus1,dgpi_3wplus2, &
dgpi_3wplus1plusbetak) !CGQ
	...
	!functional form expression
		...
		mu_MG = 1.d0 + CP%mu0 * (OmegaDE(State,a,adotoa)/omegav) * s1_k !when CP%Lambda=0 original DES parameterization is recovered
    ...
    function ISiTGR_Q(State,k,a,adotoa)
	use constants
	use results
    use classes
    class(TCAMBdata), intent(in) :: State
    ...
    if(CP%ISiTGR_BIN) then
        ISiTGR_Q = (1+ISiTGR_Q_Z1(k) +(ISiTGR_Q_Z2(k)-ISiTGR_Q_Z1(k))*tanh((1.d0/a-1.d0-CP%z_div)/CP%z_tw) &
        +(1-ISiTGR_Q_Z2(k))*tanh((1.d0/a-1.d0-CP%z_TGR)/CP%z_tw))/2.d0
		
	else if (CP%ISiTGR_QDR .eqv. .true.) then

		!adding an extra factor for scale dependence
		s2_k = (CP%lambda*(adotoa/a)/k)**2.d0
		s1_k = (1.d0+CP%c1*s2_k)/(1.d0+s2_k)
		!scale dependent
		ISiTGR_Q = 1.d0 + CP%Q0 * OmegaDE(State,a,adotoa)/omegav * s1_k
		
    else if(CP%t_dep) then
        ISiTGR_Q=(CP%Q0*dexp(-k/CP%k_c) + CP%Qinf*(1.d0-dexp(-k/CP%k_c))-1.d0)*a**CP%s+1.d0
		
	else 
	    ISiTGR_Q=CP%Q0*dexp(-k/CP%k_c) + CP%Qinf*(1.d0-dexp(-k/CP%k_c))
    ...
    function ISiTGR_Q_dot(State,k,a,adotoa,Hdot)
    use constants
	use results
    use classes
    class(TCAMBdata), intent(in) :: State
    ...
    if(CP%ISiTGR_BIN) then
        ISiTGR_Q_dot = adotoa/2.d0/CP%z_tw/a*((ISiTGR_Q_Z1(k)-ISiTGR_Q_Z2(k))/cosh((1.d0/a-1.d0-CP%z_div)/CP%z_tw)**2.d0 &
        +(ISiTGR_Q_Z2(k)-1)/cosh((1.d0/a-1.d0-CP%z_TGR)/CP%z_tw)**2.d0)
	...
	!functional form expression
		...
		s1_k_dot = s2_k_dot*(CP%c1-1.d0)/(1+s2_k)**2.d0
		!scale dependent
		ISiTGR_Q_dot = CP%Q0*(OmegaDEdot(State,a,adotoa,Hdot)/omegav*s1_k + OmegaDE(State,a,adotoa)/omegav*s1_k_dot)
    
	else if(CP%t_dep) then
        ISiTGR_Q_dot=(ISiTGR_Q(State,k,a,adotoa)-1.d0)*CP%s*adotoa
    ...
    function ISiTGR_D(State,k,a,adotoa)
	use constants
    use classes
	use results
    class(TCAMBdata), intent(in) :: State
    ...
    if(CP%ISiTGR_BIN) then
        ISiTGR_D = (1+ISiTGR_D_Z1(k) +(ISiTGR_D_Z2(k)-ISiTGR_D_Z1(k))*tanh((1.d0/a-1.d0-CP%z_div)/CP%z_tw) &
        +(1-ISiTGR_D_Z2(k))*tanh((1.d0/a-1.d0-CP%z_TGR)/CP%z_tw))/2.d0
	
	else
		
		if(CP%R_func)then
    	    ISiTGR_D = ISiTGR_Q(State,k,a,adotoa)*(1.d0+ISiTGR_DR(State,k,a,adotoa))/2.d0
		
		else
			ISiTGR_D = ISiTGR_DR(State,k,a,adotoa)
    ...
    function ISiTGR_D_dot(State,k,a,adotoa,Hdot)
	use constants
    use classes
	use results
    class(TCAMBdata), intent(in) :: State
    ...
    if(CP%ISiTGR_BIN) then
        ISiTGR_D_dot = adotoa/2.d0/CP%z_tw/a*((ISiTGR_D_Z1(k)-ISiTGR_D_Z2(k))/cosh((1.d0/a-1.d0-CP%z_div)/CP%z_tw)**2.d0 &
        +(ISiTGR_D_Z2(k)-1)/cosh((1.d0/a-1.d0-CP%z_TGR)/CP%z_tw)**2.d0)
    
	else 
	
		if(CP%R_func)then
        	ISiTGR_D_dot = ISiTGR_Q_dot(State,k,a,adotoa,Hdot)*(1.d0+ISiTGR_DR(State,k,a,adotoa))/2.d0 &
        	+ISiTGR_Q(State,k,a,adotoa)*ISiTGR_DR_dot(State,k,a,adotoa,Hdot)/2.d0

		else
			ISiTGR_D_dot = ISiTGR_DR_dot(State,k,a,adotoa,Hdot)
    ...
    function ISiTGR_DR(State,k,a,adotoa)
	use constants
	use results
    use classes
    class(TCAMBdata), intent(in) :: State
	...
	if (CP%ISiTGR_QDR .eqv. .true.) then
	
		!adding an extra factor for scale dependence
		s2_k = (CP%lambda*(adotoa/a)/k)**2.d0
		s1_k = (1.d0+CP%c2*s2_k)/(1.d0+s2_k)
		!scale dependent
		ISiTGR_DR = 1.d0 + CP%DR0 * OmegaDE(State,a,adotoa)/omegav * s1_k

	else

	    if(CP%t_dep) then
    	    ISiTGR_DR=(CP%dr0*dexp(-k/CP%k_c) + CP%drinf*(1.d0-dexp(-k/CP%k_c))-1.d0)*a**CP%s+1.d0
	    else
	        ISiTGR_DR=CP%dr0*dexp(-k/CP%k_c) + CP%drinf*(1.d0-dexp(-k/CP%k_c))
	    end if
    ...
    function ISiTGR_DR_dot(State,k,a,adotoa,Hdot)
	use constants
	use results
    use classes
    class(TCAMBdata), intent(in) :: State
	...
	if (CP%ISiTGR_QDR .eqv. .true.) then
		...
		s1_k_dot = s2_k_dot*(CP%c2-1.d0)/(1+s2_k)**2.d0
		!scale dependent
		ISiTGR_DR_dot = CP%DR0*(OmegaDEdot(State,a,adotoa,Hdot)/omegav*s1_k + OmegaDE(State,a,adotoa)/omegav*s1_k_dot)
		
	else

	    if(CP%t_dep) then
    	    ISiTGR_DR_dot=(ISiTGR_DR(State,k,a,adotoa)-1.d0)*CP%s*adotoa
	    else
	        ISiTGR_DR_dot=0.
	    end if
    ...
    if(CP%true_bin) then
        ISiTGR_Q_Z1=(CP%Q2+CP%Q1)/2.d0 + (CP%Q2-CP%Q1)/2.d0*tanh((k-CP%k_c)/CP%k_tw)
    else
        ISiTGR_Q_Z1=CP%Q1*dexp(-k/CP%k_c) + CP%Q2*(1.d0-dexp(-k/CP%k_c))
    end if
    ...
    if(CP%true_bin) then
        ISiTGR_Q_Z2=(CP%Q4+CP%Q3)/2.d0 + (CP%Q4-CP%Q3)/2.d0*tanh((k-CP%k_c)/CP%k_tw)
    else
        ISiTGR_Q_Z2=CP%Q3*dexp(-k/CP%k_c) + CP%Q4*(1.d0-dexp(-k/CP%k_c))
    end if
    ...
    if(CP%true_bin) then
        ISiTGR_D_Z1=(CP%D2+CP%D1)/2.d0 + (CP%D2-CP%D1)/2.d0*tanh((k-CP%k_c)/CP%k_tw)
    else
        ISiTGR_D_Z1=CP%D1*dexp(-k/CP%k_c) + CP%D2*(1.d0-dexp(-k/CP%k_c))
    end if
    ...
    if(CP%true_bin) then
        ISiTGR_D_Z2=(CP%D4+CP%D3)/2.d0 + (CP%D4-CP%D3)/2.d0*tanh((k-CP%k_c)/CP%k_tw)
    else
        ISiTGR_D_Z2=CP%D3*dexp(-k/CP%k_c) + CP%D4*(1.d0-dexp(-k/CP%k_c))
    end if
    ...
    if(CP%true_bin) then
        ISiTGR_mu_Z1=(CP%mu2+CP%mu1)/2.d0 + (CP%mu2-CP%mu1)/2.d0*tanh((k-CP%k_c)/CP%k_tw)
    else
        ISiTGR_mu_Z1=CP%mu1*dexp(-k/CP%k_c) + CP%mu2*(1.d0-dexp(-k/CP%k_c))
    end if
    ...
    if(CP%true_bin) then
        ISiTGR_mu_Z2=(CP%mu4+CP%mu3)/2.d0 + (CP%mu4-CP%mu3)/2.d0*tanh((k-CP%k_c)/CP%k_tw)
    else
        ISiTGR_mu_Z2=CP%mu3*dexp(-k/CP%k_c) + CP%mu4*(1.d0-dexp(-k/CP%k_c))
    end if
    ...
    if(CP%true_bin) then
        ISiTGR_eta_Z1=(CP%eta2+CP%eta1)/2.d0 + (CP%eta2-CP%eta1)/2.d0*tanh((k-CP%k_c)/CP%k_tw)
    else
        ISiTGR_eta_Z1=CP%eta1*dexp(-k/CP%k_c) + CP%eta2*(1.d0-dexp(-k/CP%k_c))
    end if
    ...
    if(CP%true_bin) then
        ISiTGR_eta_Z2=(CP%eta4+CP%eta3)/2.d0 + (CP%eta4-CP%eta3)/2.d0*tanh((k-CP%k_c)/CP%k_tw)
    else
        ISiTGR_eta_Z2=CP%eta3*dexp(-k/CP%k_c) + CP%eta4*(1.d0-dexp(-k/CP%k_c))
    end if
    ...
    if(CP%true_bin) then
        ISiTGR_Sigma_Z1=(CP%Sigma2+CP%Sigma1)/2.d0 + (CP%Sigma2-CP%Sigma1)/2.d0*tanh((k-CP%k_c)/CP%k_tw)
    else
        ISiTGR_Sigma_Z1=CP%Sigma1*dexp(-k/CP%k_c) + CP%Sigma2*(1.d0-dexp(-k/CP%k_c))
    end if
    ...
    if(CP%true_bin) then
        ISiTGR_Sigma_Z2=(CP%Sigma4+CP%Sigma3)/2.d0 + (CP%Sigma4-CP%Sigma3)/2.d0*tanh((k-CP%k_c)/CP%k_tw)
    else
        ISiTGR_Sigma_Z2=CP%Sigma3*dexp(-k/CP%k_c) + CP%Sigma4*(1.d0-dexp(-k/CP%k_c))
    end if
    ...
    real(dl) :: OmegaDE, omegav, HubbleConstant
...
!write(*,*) CP%E11, CP%E22, CP%c1, CP%c2, CP%lambda, CP%ISiTGR_mueta, CP%ISiTGR_muSigma, CP%GR
	...
	dgpi_3wplus1plusbetak = 0.d0
		...
		if ((CP%ISiTGR_mueta .eqv. .true.) .or. (CP%ISiTGR_BIN_mueta .eqv. .true.)) then
		...
		else if ((CP%ISiTGR_muSigma .eqv. .true.) .or. (CP%ISiTGR_BIN_muSigma .eqv. .true.)) then
    		...
    		TGR_Q_T=ISiTGR_Q(State,k,a,adotoa)
		    TGR_Q_T_dot=ISiTGR_Q_dot(State,k,a,adotoa,Hdot)
		    TGR_D_T= ISiTGR_D(State,k,a,adotoa)
		    TGR_D_T_dot=ISiTGR_D_dot(State,k,a,adotoa,Hdot)		
	...
	! 3) Get sigmadot
		if ((CP%ISiTGR_mueta .eqv. .true.) .or. (CP%ISiTGR_muSigma .eqv. .true.) &
			.or. (CP%ISiTGR_BIN_mueta .eqv. .true.) .or. (CP%ISiTGR_BIN_muSigma .eqv. .true.)) then
    ...
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        if (associated(EV%OutputSources)) then
        	...
        	if (CP%GR==0) then
		        ...
		        if ((CP%ISiTGR_mueta .eqv. .true.) .or. (CP%ISiTGR_muSigma .eqv. .true.) &
					.or. (CP%ISiTGR_BIN_mueta .eqv. .true.) .or. (CP%ISiTGR_BIN_muSigma .eqv. .true.)) then !CGQ for ISW effect for mueta, muSigma parameterizations
