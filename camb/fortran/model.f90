    ...
    type, extends (TCAMBParameters) :: CAMBparams
		...
		!> ISiTGR MOD START
        real(dl) :: Q0=0._dl
		real(dl) ::Qinf=0._dl	!models 3-6
        real(dl) :: DR0=0._dl
		real(dl) :: DRinf=0._dl !models 3-6
        real(dl) :: s=0._dl !models 3, 5
        real(dl) :: k_c=0._dl ! The k value where we change k-bins for true binning !models 1-6
        real(dl) :: Q1=0._dl
		real(dl) :: Q2=0._dl
		real(dl) :: Q3=0._dl
		real(dl) :: Q4=0._dl !binning models 1 and 2
        real(dl) :: D1=0._dl
		real(dl) :: D2=0._dl
		real(dl) :: D3=0._dl
		real(dl) :: D4=0._dl !binning models 1 and 2
        real(dl) :: z_div=1._dl !This is the size of each z-bin !models 1 and 2
        real(dl) :: z_TGR=1._dl !The redshift below which we test GR usueally 2*z_div !models 1 and 2
        real(dl) :: z_tw=1._dl  !transition width between redshift bins !models 1 and 2
        real(dl) :: k_tw=1._dl  !transition width between k-bins. !models 1 and 2
        logical :: t_dep=.false. 
        logical :: R_func=.false.
        logical  :: true_bin=.false.  
        logical  :: ISiTGR_BIN=.false.
        !CGQ ------------------------------------------------
		!Functional Form Parameters
		real(dl) :: E11=0._dl
		real(dl) :: E22=0._dl !For mu, eta Planck's parameterization !functional form
		real(dl) :: mu0=0._dl
		real(dl) :: Sigma0=0._dl !For mu, Sigma DES parameterization !functional form
		!For scale-dependence
		real(dl) :: c1=1._dl
		real(dl) :: c2=1._dl
		real(dl) :: lambda=0._dl
		!Binning Form Parameters
		real(dl) :: mu1=0._dl
		real(dl) :: mu2=0._dl
		real(dl) :: mu3=0._dl
		real(dl) :: mu4=0._dl !binning models 
        real(dl) :: eta1=0._dl
		real(dl) :: eta2=0._dl
		real(dl) :: eta3=0._dl
		real(dl) :: eta4=0._dl !binning models 
