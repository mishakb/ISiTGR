	...
	!> ISiTGR MOD START: to read ISiTGR parameters in equations_ISiTGR.f90
...
!	Scale_dep = Ini%Read_Logical('Scale_dep', .false.)
	Monoto_evol = Ini%Read_Logical('Monoto_evol', .false.)
	Binning_Method = Ini%Read_Logical('Binning_Method', .false.)
	Hybrid_Binning = Ini%Read_Logical('Hybrid_Binning', .false.)
...
if (Binning_Method .eqv. .true.) then
	...
	P%k_c = Ini%Read_Double('k_c_BIN', 0.d0)
	P%z_div = Ini%Read_Double('z_div',0.d0)
	P%z_TGR = Ini%Read_Double('z_TGR',0.d0)
	P%z_tw = Ini%Read_Double('z_tw',0.d0)
	...
	!mu-eta
		...
		P%ISiTGR_BIN_mueta = .true.
		P%mu1 = Ini%Read_Double('mu1',0.d0)
		P%mu2 = Ini%Read_Double('mu2',0.d0)
		P%mu3 = Ini%Read_Double('mu3',0.d0)
		P%mu4 = Ini%Read_Double('mu4',0.d0)
		P%eta1 = Ini%Read_Double('eta1',0.d0)
		P%eta2 = Ini%Read_Double('eta2',0.d0)
		P%eta3 = Ini%Read_Double('eta3',0.d0)
		P%eta4 = Ini%Read_Double('eta4',0.d0)
		write(*,*) 'ISiTGR: Binning method for mu-eta'
	
	!mu-Sigma
	else if (Parameterization_BIN == 2) then
		P%ISiTGR_BIN_muSigma = .true.
		P%mu1 = Ini%Read_Double('mu1',0.d0)
		P%mu2 = Ini%Read_Double('mu2',0.d0)
		P%mu3 = Ini%Read_Double('mu3',0.d0)
		P%mu4 = Ini%Read_Double('mu4',0.d0)
		P%Sigma1 = Ini%Read_Double('Sigma1',0.d0)
		P%Sigma2 = Ini%Read_Double('Sigma2',0.d0)
		P%Sigma3 = Ini%Read_Double('Sigma3',0.d0)
		P%Sigma4 = Ini%Read_Double('Sigma4',0.d0)
		write(*,*) 'ISiTGR: Binning method for mu-Sigma'
	
	!Q-D and Q-R
	else if (Parameterization_BIN == 3) then
		P%ISiTGR_BIN = .true.
		P%Q1 = Ini%Read_Double('Q1',0.d0)
		P%Q2 = Ini%Read_Double('Q2',0.d0)
		P%Q3 = Ini%Read_Double('Q3',0.d0)
		P%Q4 = Ini%Read_Double('Q4',0.d0)
		P%D1 = Ini%Read_Double('D1',0.d0)
		P%D2 = Ini%Read_Double('D2',0.d0)
		P%D3 = Ini%Read_Double('D3',0.d0)
		P%D4 = Ini%Read_Double('D4',0.d0)
	...
	if (Hybrid_Binning .eqv. .true.) then
		...
		P%true_bin = .true.
		P%k_tw = Ini%Read_Double('k_tw',0.d0)
	...
	P%ISiTGR_muSigma = .false.
	
	!read scale dependence parameters
	P%c1 = Ini%Read_Double('c1', 0.d0)
	P%c2 = Ini%Read_Double('c2', 0.d0)
	P%lambda = Ini%Read_Double('lambda', 0.d0)
	if (P%lambda /= 0) then
	...
	!for mu and eta parameterization
		...
		P%E11 = Ini%Read_Double('E11')
		P%E22 = Ini%Read_Double('E22')
		write(*,*) 'E11=', P%E11, 'E22=', P%E22
	
	!for mu and Sigma parameterization
	else if (Parameterization == 2) then
		P%ISiTGR_muSigma = .true.
		write(*,*) '        DES parametrization'
		P%mu0 = Ini%Read_Double('mu0', 0.d0)
		P%Sigma0 = Ini%Read_Double('Sigma0', 0.d0)
		write(*,*) 'mu0=', P%mu0, 'Sigma0=', P%Sigma0
	...
	!for MG parameters Q, D
		...
		if (Monoto_evol .eqv. .true.) then	
			P%ISiTGR_QDR = .false.
			P%Q0 = Ini%Read_Double('Q0',0.d0)
			P%Qinf = Ini%Read_Double('Qinf',0.d0)
			P%DR0 = Ini%Read_Double('DR0',0.d0)
			P%DRinf = Ini%Read_Double('DRinf',0.d0)
			P%k_c = Ini%Read_Double('k_c', 0.d0)
			!including time dependence through s parameter
			if (exp_s_dep .eqv. .true.) then
				P%t_dep = .true.
				P%s = Ini%Read_Double('s_dep',0.d0)
			end if
		else	
			P%ISiTGR_QDR = .true.
			P%Q0 = Ini%Read_Double('QD0',0.d0)
			P%DR0 = Ini%Read_Double('D0',0.d0)
			write(*,*) 'Q0=', P%Q0, 'D0=', P%DR0
		end if
	...
	!for MG parameters Q, R
		...
		if (Monoto_evol .eqv. .true.) then	
			...
			P%ISiTGR_QDR = .true.
			P%Q0 = Ini%Read_Double('QR0',0.d0)
			P%DR0 = Ini%Read_Double('R0',0.d0)
			write(*,*) 'Q0=', P%Q0, 'R0=', P%DR0
		end if
