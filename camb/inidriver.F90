    ...
    use CAMBmain
    use NonLinear
	!>ISiTGR MOD START: to read ISiTGR parameters in equations.f90
	!CGQ
	use ISiTGR
	!<ISiTGR MOD END
#ifdef NAGF95
	...
	!>ISiTGR MOD START: to read ISiTGR parameters in equations.f90
	!CGQ
	integer :: Parameterization, Parameterization_BIN !to go through different parameterizations
	...
	!>ISiTGR MOD START: read MG parameters for different parameterizations -------------------------------------------------

	!CGQ
	Parameterization = Ini_Read_Int('Parameterization', 0)
	TGR%GR = 1
	Parameterization_BIN = Ini_Read_Int('Parameterization_BIN', 0)
	exp_s_dep = Ini_Read_logical('exp_s_dep', .false.)
!	Scale_dep = Ini_Read_logical('Scale_dep', .false.)
	Monoto_evol = Ini_Read_logical('Monoto_evol', .false.)
	Binning_Method = Ini_Read_logical('Binning_Method', .false.)
	Hybrid_Binning = Ini_Read_logical('Hybrid_Binning', .false.)

	!CGQ
	if (Parameterization /= 0 .and. Parameterization /= 1 .and. Parameterization /= 2 .and. Parameterization /= 3 &
	.and. Parameterization /= 4 ) then
		print*, '***please choose a parameterization***'
		stop
	end if
	if (Parameterization_BIN /= 0 .and. Parameterization_BIN /= 1 .and. Parameterization_BIN /= 2 .and. &
	Parameterization_BIN /= 3) then
		print*, '***please choose a parameterization_BIN***'
		stop
	end if
	
	write(*,*) "---------------------"
	if (Binning_Method .eqv. .false.) then
		write(*,*) 'ISiTGR: Functional form for MG parameters'
		write(*,*) "Parameterization : ", Parameterization
	else 
		write(*,*) 'ISiTGR: Binning form for MG parameters'
		write(*,*) "Parameterization_BIN : ", Parameterization_BIN
	end if
	write(*,*) "---------------------"
	
if (Binning_Method .eqv. .true.) then

	TGR%k_c = Ini_Read_Double('k_c_BIN', 0.d0)
	TGR%z_div = Ini_Read_Double('z_div',0.d0)
	TGR%z_TGR = Ini_Read_Double('z_TGR',0.d0)
	TGR%z_tw = Ini_Read_Double('z_tw',0.d0)
	...
	!mu-eta
		...
		TGR%ISiTGR_BIN_mueta = .true.
		TGR%mu1 = Ini_Read_Double('mu1',0.d0)
		TGR%mu2 = Ini_Read_Double('mu2',0.d0)
		TGR%mu3 = Ini_Read_Double('mu3',0.d0)
		TGR%mu4 = Ini_Read_Double('mu4',0.d0)
		TGR%eta1 = Ini_Read_Double('eta1',0.d0)
		TGR%eta2 = Ini_Read_Double('eta2',0.d0)
		TGR%eta3 = Ini_Read_Double('eta3',0.d0)
		TGR%eta4 = Ini_Read_Double('eta4',0.d0)
		write(*,*) 'ISiTGR: Binning method for mu-eta'
	
	!mu-Sigma
	else if (Parameterization_BIN == 2) then
		TGR%ISiTGR_BIN_muSigma = .true.
		TGR%mu1 = Ini_Read_Double('mu1',0.d0)
		TGR%mu2 = Ini_Read_Double('mu2',0.d0)
		TGR%mu3 = Ini_Read_Double('mu3',0.d0)
		TGR%mu4 = Ini_Read_Double('mu4',0.d0)
		TGR%Sigma1 = Ini_Read_Double('Sigma1',0.d0)
		TGR%Sigma2 = Ini_Read_Double('Sigma2',0.d0)
		TGR%Sigma3 = Ini_Read_Double('Sigma3',0.d0)
		TGR%Sigma4 = Ini_Read_Double('Sigma4',0.d0)
		write(*,*) 'ISiTGR: Binning method for mu-Sigma'
	
	!Q-D and Q-R
	else if (Parameterization_BIN == 3) then
		TGR%ISiTGR_BIN = .true.
		TGR%Q1 = Ini_Read_Double('Q1',0.d0)
		TGR%Q2 = Ini_Read_Double('Q2',0.d0)
		TGR%Q3 = Ini_Read_Double('Q3',0.d0)
		TGR%Q4 = Ini_Read_Double('Q4',0.d0)
		TGR%D1 = Ini_Read_Double('D1',0.d0)
		TGR%D2 = Ini_Read_Double('D2',0.d0)
		TGR%D3 = Ini_Read_Double('D3',0.d0)
		TGR%D4 = Ini_Read_Double('D4',0.d0)
		write(*,*) 'ISiTGR: Binning method for Q-D'

	end if

	!whether to use hybrid binning or traditional binning 
	if (Hybrid_Binning .eqv. .true.) then
		TGR%true_bin = .false.
		write(*,*) 'Hybrid method'
	else 
		TGR%true_bin = .true.
		TGR%k_tw = Ini_Read_Double('k_tw',0.d0)
		write(*,*) 'Traditional Binning method'
	end if
	
else
	TGR%ISiTGR_BIN = .false.
	TGR%ISiTGR_mueta = .false.
	TGR%ISiTGR_muSigma = .false.
	
	!read scale dependence parameters
	TGR%c1 = Ini_Read_Double('c1', 0.d0)
	TGR%c2 = Ini_Read_Double('c2', 0.d0)
	TGR%lambda = Ini_Read_Double('lambda', 0.d0)
	if (TGR%lambda /= 0) then
	write(*,*) 'ISiTGR: adding scale dependence'
	end if
	...
	!for mu and eta parameterization
		...
		TGR%ISiTGR_mueta = .true.
		write(*,*) '        Planck parametrization'
		TGR%E11 = Ini_Read_Double('E11', 0.d0)
		TGR%E22 = Ini_Read_Double('E22', 0.d0)
		write(*,*) 'E11=', TGR%E11, 'E22=', TGR%E22
	
	!for mu and Sigma parameterization
	else if (Parameterization == 2) then
		TGR%ISiTGR_muSigma = .true.
		write(*,*) '        DES parametrization'
		TGR%mu0 = Ini_Read_Double('mu0', 0.d0)
		TGR%Sigma0 = Ini_Read_Double('Sigma0', 0.d0)
		write(*,*) 'mu0=', TGR%mu0, 'Sigma0=', TGR%Sigma0
	
	!for MG parameters Q, D
	else if (Parameterization == 3) then
		TGR%R_func = .false.
		write(*,*) '        Q, D parameterization'

		if (Monoto_evol .eqv. .true.) then	
			TGR%ISiTGR_QDR = .false.
			TGR%Q0 = Ini_Read_Double('Q0',0.d0)
			TGR%Qinf = Ini_Read_Double('Qinf',0.d0)
			TGR%DR0 = Ini_Read_Double('DR0',0.d0)
			TGR%DRinf = Ini_Read_Double('DRinf',0.d0)
			TGR%k_c = Ini_Read_Double('k_c', 0.d0)
			!including time dependence through s parameter
			if (exp_s_dep .eqv. .true.) then
				TGR%t_dep = .true.
				TGR%s = Ini_Read_Double('s_dep',0.d0)
			end if
		else	
			TGR%ISiTGR_QDR = .true.
			TGR%Q0 = Ini_Read_Double('QD0',0.d0)
			TGR%DR0 = Ini_Read_Double('D0',0.d0)
			write(*,*) 'Q0=', TGR%Q0, 'D0=', TGR%DR0
		end if
		
	!for MG parameters Q, R
	else if (Parameterization == 4) then
		TGR%R_func = .true.
		write(*,*) '        Q, R parameterization'
		...
		if (Monoto_evol .eqv. .true.) then	
			...
			TGR%ISiTGR_QDR = .true.
			TGR%Q0 = Ini_Read_Double('QR0',0.d0)
			TGR%DR0 = Ini_Read_Double('R0',0.d0)
			write(*,*) 'Q0=', TGR%Q0, 'R0=', TGR%DR0
		end if
	...
	TGR%DE_eqstate = Ini_Read_Int('DarkEnergyModel', 0)
	if (TGR%DE_eqstate == 0) then
		TGR%w0 = -1.d0
	else if (TGR%DE_eqstate == 1) then
		TGR%w0 = Ini_Read_Double('wDE0',0.d0)
	else if (TGR%DE_eqstate == 2) then
		TGR%w0 = Ini_Read_Double('wDE0',0.d0)
		TGR%wa = Ini_Read_Double('wDEa',0.d0)
	else if (TGR%DE_eqstate == 3) then
		TGR%wa = Ini_Read_Double('wDEa',0.d0)
		TGR%wp = Ini_Read_Double('wDEp',0.d0)
		TGR%a_p = Ini_Read_Double('ap',0.d0)
	end if
