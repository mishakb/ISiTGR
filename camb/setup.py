...
def find_version():
    version_file = io.open(os.path.join(file_dir, 'isitgr', '__init__.py')).read()
...
def make_library(cluster=False):
    ...
    if _compile.is_windows:
        ...
        subprocess.call("make python PYCAMB_OUTPUT_DIR=%s/isitgr/ CLUSTER_SAFE=%d" %
...
if __name__ == "__main__":
    setup(name=os.getenv('CAMB_PACKAGE_NAME', 'isitgr'),
          ...
          platforms="any",
          package_data={'isitgr': [DLLNAME, 'HighLExtrapTemplate_lenspotentialCls.dat',
                                 'PArthENoPE_880.2_marcucci.dat', 'PArthENoPE_880.2_standard.dat',
                                 'PRIMAT_Yp_DH_Error.dat']},
          test_suite='isitgr.tests',
          entry_points={
              'console_scripts': [
