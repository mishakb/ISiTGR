...
typedef struct {
  ...
  double TGR_Q1,TGR_Q2,TGR_Q3,TGR_Q4;
  double TGR_D1,TGR_D2,TGR_D3,TGR_D4;
  double k_c,k_tw,z_TGR,z_tw,z_div;
  int real_bin;
  /*JD for Modified save tables for r(chi) and as well as P(k) interpolation*/
  int pk_initialized,is_initialized;
  double Pktab[500][530],Pk2tab[500][530],kvec[530];
...
void DNread_cosmo(COSMOPARAM *p, char FileName[]) {
  ...
  /*JD*/
  fscanf(fp, "%lg", &(p->TGR_Q1));
  fscanf(fp, "%lg", &(p->TGR_D1));
  fscanf(fp, "%lg", &(p->TGR_Q2));
  fscanf(fp, "%lg", &(p->TGR_D2));
  fscanf(fp, "%lg", &(p->TGR_Q3));
  fscanf(fp, "%lg", &(p->TGR_D3));
  fscanf(fp, "%lg", &(p->TGR_Q4));
  fscanf(fp, "%lg", &(p->TGR_D4));
  fscanf(fp, "%lg", &(p->z_div));
  fscanf(fp, "%lg", &(p->k_c));
  fscanf(fp,  "%d", &(p->real_bin));
  fclose(fp);
  p->z_TGR = 2.0*p->z_div;
  p->k_tw = p->k_c/10.0;
  p->z_tw = 0.05;
...
double geff(COSMOPARAM *p, double k, double z){
  double TGR_Q,TGR_D,TGR_Q_Z1,TGR_Q_Z2,TGR_D_Z1,TGR_D_Z2;
  
  if(p->real_bin == 1){
    TGR_Q_Z1 = (p->TGR_Q2+p->TGR_Q1)/2.0 + (p->TGR_Q2-p->TGR_Q1)/2.0*tanh((k-p->k_c)/p->k_tw);
    TGR_Q_Z2 = (p->TGR_Q4+p->TGR_Q3)/2.0 + (p->TGR_Q4-p->TGR_Q3)/2.0*tanh((k-p->k_c)/p->k_tw);
    
    TGR_D_Z1 = (p->TGR_D2+p->TGR_D1)/2.0 + (p->TGR_D2-p->TGR_D1)/2.0*tanh((k-p->k_c)/p->k_tw);
    TGR_D_Z2 = (p->TGR_D4+p->TGR_D3)/2.0 + (p->TGR_D4-p->TGR_D3)/2.0*tanh((k-p->k_c)/p->k_tw);
  }
  else{
    TGR_Q_Z1=p->TGR_Q1*exp(-k/p->k_c) + p->TGR_Q2*(1.0-exp(-k/p->k_c));
    TGR_Q_Z2=p->TGR_Q3*exp(-k/p->k_c) + p->TGR_Q4*(1.0-exp(-k/p->k_c));
    
    TGR_D_Z1=p->TGR_D1*exp(-k/p->k_c) + p->TGR_D2*(1.0-exp(-k/p->k_c));
    TGR_D_Z2=p->TGR_D3*exp(-k/p->k_c) + p->TGR_D4*(1.0-exp(-k/p->k_c));
  }
        
  TGR_Q=(1+TGR_Q_Z1 +(TGR_Q_Z2-TGR_Q_Z1)*tanh((z-p->z_div)/p->z_tw)+
          (1-TGR_Q_Z2)*tanh((z-p->z_TGR)/p->z_tw))/2.0;
              
  TGR_D=(1+TGR_D_Z1 +(TGR_D_Z2-TGR_D_Z1)*tanh((z-p->z_div)/p->z_tw)+
          (1-TGR_D_Z2)*tanh((z-p->z_TGR)/p->z_tw))/2.0;

  return(2.0*TGR_D - TGR_Q);
}
...
double DNget_chi(COSMOPARAM *p, double z1, double z2) {
  ...
  if (!p->is_initialized) {
    p->is_initialized=1;
    for(i=0;i<=1000;i++)
      p->distances[i] = DNget_chi11(p,0.,0.005*i);
  }
  ...
  if (ii1>=0 && ii1<=1000 && ii2>=0 && ii2<=1000 && fabs(ii1-i1)<1e-7 && fabs(ii2-i2)<1e-7){
    return(p->distances[i2]-p->distances[i1]);
  }
  else{
...
/* Growth function derivative */  /*JD added k*/
void DNgrowth_derivative(COSMOPARAM *p,double k, double lny, double *X, double *dXdy, int flag) {
  ...
  dXdy[0] = X[1]/H+X[0];
  dXdy[1] = 3*X[1]+1.5*geff(p,k,z)*omH*X[0];  /*JD added geff for testing GR*/
}
 /*JD added k*/
void DNgrowth_step(COSMOPARAM *p,double k, double lny, double *Xin, double *Xout, double dlny, int flag) {
  ...
  /* Get derivatives */
  DNgrowth_derivative(p,k, lny, Xin, k1, flag);

  for(i=0;i<2;i++) X[i] = Xin[i]+half_dlny*k1[i];
  DNgrowth_derivative(p,k, lnyh, X, k2, 1);

  for(i=0;i<2;i++) X[i] = Xin[i]+half_dlny*k2[i];
  DNgrowth_derivative(p,k, lnyh, X, k3, 0);

  for(i=0;i<2;i++) X[i] = Xin[i]+dlny*k3[i];
  DNgrowth_derivative(p,k, lny+dlny, X, k4, 1);

  for(i=0;i<2;i++) Xout[i] = Xin[i] + dlny/6.*(k1[i]+2*(k2[i]+k3[i])+k4[i]);
}

/* Growth Function*/
double DNget_growth_normhiz(COSMOPARAM *p,double k, double z) {
  ...
  for(i=0; i<N; i++) {
    DNgrowth_step(p,k, lny, Xold, Xnew, dlny, i? 0: 1);
    lny += dlny;
...
double DNget_growth_normtoday(COSMOPARAM *p,double k, double z) {
  return(DNget_growth_normhiz(p,k,z)/DNget_growth_normhiz(p,k,0));
}
...
/*JD changed for more efficiently calculating using interpolation since growth is a function of 
k and z*/  /*changed double z to int z since z values are fixed in table no need for z interpolation*/
/* Normalized linear power spectrum */
double DNpk_lin(COSMOPARAM *p, double k, int z) {
  static int nk = 530;
  int nz = 500;
  int i,j;
  double zstep = 0.01;
  double dlnk = 0.023;
  double expdlnk,z2,k2,Pk;
  expdlnk = exp(dlnk);
  ...
  if (!p->pk_initialized) {
    ...
    for(j=0;j<nk;j++){
      for(i=0;i<nz;i++){
        z2 = (i+0.5)*zstep;
        p->Pktab[i][j]=p->A*DNpk_unnorm(p,k2,z2);
      }
      p->kvec[j]=k2;  
      k2*=expdlnk;
    }
    for(i=0;i<nz;i++){
      spline(p->kvec,p->Pktab[i],nk,1e30,1e30,p->Pk2tab[i]);
    }
  }
  
  splint(p->kvec,p->Pktab[z],p->Pk2tab[z],nk,k,&Pk);
  return(Pk); 
