...
/*JD for Testing GR This function is the factor by which the lensing kernel
  will be multiplied by to Test GR (this comes from a modified definition 
  of the convergence*/
double TGR_D(COSMOPARAM *p, double z){
	double TGR_dr,TGR_q,a;
	a = 1.0/(1.0+z);
...
double lensingkernel(double zl, double zs, COSMOPARAM *p) {
  double W;
  if (zl>=zs) return(0.);
//JD edited below for testing GR: convergence has a factor of TGR_D in it
// For scale independent we put that factor here. 
  W = TGR_D(p,zl)*1.5 * p->omega_M * DNget_r(p,0,zl) * DNget_r(p,zl,zs) * (1.+zl) / DNget_r(p,0,zs) / DNhubble(p,zl) / HL_HIMPC;
  return(W);
}
...
void repairdndz(double *dist_in, double *dist_out, double alpha, COSMOPARAM *p, int flag) {
  ...
  for(i=0; i<NZ; i++) {
    ...
    for(j=i+1; j<NZ; j++)
      if (dist_in[j]!=0)
        dist_temp[i] += alpha*lensingkernel((i+0.5)*ZSTEP,(j+0.5)*ZSTEP,p)*dist_in[j];
  }
...
void angularpower(double L, double *dist1, double *dist2, COSMOPARAM *p, double *out0, double *out1) {
  ...
  for(i=0; i<NZ; i++) {
    ...
    if (prod!=0) {
      ...
      k = L/r;
      Pk = out1==NULL? DNpk_nonlin(p,L/r,z): DNpk_lin(p,L/r,z);
      if (out1!=NULL && out1!=out0) {
        Pk /= (1.+1.7*k);
...
void construct_dist_qso(COSMOPARAM *p, double **dist) {
  ...
  norm0 = DNget_growth_normhiz(p,0);
  for(iz=0; iz<NZ; iz++) {
    z = (iz+0.5)*ZSTEP;
    growth = DNget_growth_normhiz(p,z)/norm0;
    for(doffset=0; doffset<2; doffset++)
