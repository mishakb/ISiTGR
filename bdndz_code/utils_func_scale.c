...
typedef struct {
  ...
  double TGR_Q0,TGR_DR0,TGR_Qinf,TGR_DRinf,TGR_kc, TGR_s;
...
void DNread_cosmo(COSMOPARAM *p, char FileName[]) {
  ...
  fscanf(fp, "%lg", &(p->TGR_Qinf));
  fscanf(fp, "%lg", &(p->TGR_DRinf));
  fscanf(fp, "%lg", &(p->TGR_kc));
...
double geff(COSMOPARAM *p, double k, double z){
  ...
  if(p->TGRTDEP==0){
    TGR_q=p->TGR_Q0*exp(-k/p->TGR_kc) + p->TGR_Qinf*(1.0-exp(-k/p->TGR_kc));
    TGR_dr=p->TGR_DR0*exp(-k/p->TGR_kc) + p->TGR_DRinf*(1.0-exp(-k/p->TGR_kc));
  }
  else{
    TGR_q=(p->TGR_Q0*exp(-k/p->TGR_kc) + p->TGR_Qinf*(1.0-exp(-k/p->TGR_kc))-1.0)*pow(a,p->TGR_s)+1.0;
    TGR_dr=(p->TGR_DR0*exp(-k/p->TGR_kc) + p->TGR_DRinf*(1.0-exp(-k/p->TGR_kc))-1.0)*pow(a,p->TGR_s)+1.0;
  }
  if(p->TGR_RFUNC ==1){
    return(TGR_q*TGR_dr);
  }
